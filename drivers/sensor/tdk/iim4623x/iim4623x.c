...
static int iim4623x_prepare_cmd(uint8_t cmd_type, uint8_t *payload, size_t payload_len,
				uint8_t *out_buf, size_t out_buf_len)
{
	uint8_t *out_buf_head = out_buf;

	/* Prepare preamble and postamble */
	...
	/* Calculate checksum and correct endianess for the wire */
...
static int iim4623x_check_ack(uint8_t *buf)
{
	...
	if (packet->ack.error_code != IIM4623X_CMD_ACK) {
			     ...
			     size_t len)
{
	...
	/*
	 * Calculate the response packet size by adding preamble, packet_number, read_user_reg
	 * payload adjusted for the actual read_len, and postamble
	 */
	const uint8_t rx_pck_len = sizeof(struct iim4623x_pck_preamble) + 2 +
				   sizeof(struct iim4623x_pck_resp_read_user_reg) - 72 + len +
				   sizeof(struct iim4623x_pck_postamble);
	uint16_t checksum;
	int ret;

	ret = iim4623x_prepare_cmd(IIM4623X_CMD_READ_USER_REGISTER, cmd, ARRAY_SIZE(cmd),
				   data->tx_buf, ARRAY_SIZE(data->tx_buf));
	if (ret < 0) {
		LOG_ERR("Preparing cmd, ret: %d", ret);
		return ret;
	}

	ret = iim4623x_bus_write_then_read(dev, data->tx_buf, ret, data->rx_buf, rx_pck_len);
	if (ret) {
	...
	/* Parse reply */
	packet = (struct iim4623x_pck_resp *)data->rx_buf;
...
static int iim4623x_write_reg(const struct device *dev, uint8_t reg, const uint8_t *buf, size_t len)
{
	...
	ret = iim4623x_prepare_cmd(IIM4623X_CMD_WRITE_USER_REGISTER, cmd, len + 4, data->tx_buf,
				   ARRAY_SIZE(data->tx_buf));
	if (ret < 0) {
		LOG_ERR("Preparing write user register command, ret: %d", ret);
		return ret;
	}

	ret = iim4623x_bus_write_then_read(dev, data->tx_buf, ret, data->rx_buf,
	...
	ret = iim4623x_check_ack(data->rx_buf);
	if (ret) {
...
static int iim4623x_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	ret = iim4623x_read_data_reg(dev, IIM4623X_REG_SAMPLE_STATUS,
...
#define IIM4623X_DT_ACCEL_FS_16    0x0
#define IIM4623X_DT_ACCEL_FS_8     0x1
#define IIM4623X_DT_ACCEL_FS_4     0x2
#define IIM4623X_DT_ACCEL_FS_2     0x3
#define IIM4623X_DT_ACCEL_FS(inst) CONCAT(IIM4623X_DT_ACCEL_FS_, DT_INST_PROP_OR(inst, accel_fs, 8))

#define IIM4623X_DT_GYRO_FS_2000  0x0
#define IIM4623X_DT_GYRO_FS_1000  0x1
#define IIM4623X_DT_GYRO_FS_480   0x2
#define IIM4623X_DT_GYRO_FS_250   0x3
#define IIM4623X_DT_GYRO_FS(inst) CONCAT(IIM4623X_DT_GYRO_FS_, DT_INST_PROP_OR(inst, gyro_fs, 480))
